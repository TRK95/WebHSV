import { apiApproveEvent, apiGetMemberEvent, apiJoinEvent } from "@/api/eventsApi";
...
const ModalMemberManage = (props: ModalProps & {
    ...
    const [userJoinEvent, setUserJoinEvent] = useState<UserInfoI | null>();
    ...
    const [member, setMember] = useState<EventMember>()
    ...
    const convertToTable = (data: EventMember) => {
    ...
    const handleAddMember = async () => {
        setLoadingJoin(true)
        // if (idUserJoinEvent && value?.id) {
        if (userJoinEvent && value?._id) {
            try {
                const data = await apiJoinEvent({
                    eventId: value?._id || "0",
                    note: '',
                    status: 1
                }, { user: userJoinEvent })
    ...
    const handleAcceptMember = async (member: EventMember | undefined, index: number) => {
        setIndexApprove(index)
        setLoadingApprove(true)
        // if (value?.id && member?.userId) {
        if (value?._id && member?.userId) {
            try {
                const data = await apiApproveEvent({
                    // eventId: value?.id,
                    eventId: value?._id,
    ...
    const handleRemoveMember = async (member: EventMember | undefined) => {
        setLoadingClubMembers(true)
        // if (value?.id && member?.userId) {
        if (value?._id && member?.userId) {
            try {
    ...
    const handleUndoMember = async (member: EventMember | undefined) => {
        setLoadingClubMembers(true)
        // if (value?.id && member?.userId) {
        if (value?._id && member?.userId) {
            try {
    ...
    const columns: ColumnsType<DataType> = [
        ...
        },
        {
            ...
            width: '12%',
            align: 'center',
            render: (text: EventMember, _, index) => {
        ...
        },
        {
            ...
            dataIndex: 'value',
            align: 'center',
            render: (text: EventMember, record, index) => (
                <Space size="small" >
