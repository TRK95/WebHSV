...
import { DOMAIN_ID_ALUMNI, PAGE_SIZE, RESPONSE_FAILED, RESPONSE_SUCCESS, statuses, STATUS_DELETED, STATUS_PRIVATE, STATUS_PUBLIC, STATUS_OPEN, DOMAIN_ID_CDBK, CONTENT_TYPE_NORMAL, CONTENT_TYPE_INTRODUCE, CONTENT_TYPE_SV5T } from '@/utils/contrants';
...
function Sv5tPageView() {
                                ...
                                if (category !== "0") {
                                    try {
                                        ...
                                        if (categoryOfnews.status === RESPONSE_SUCCESS) {
                                            ...
                                            notification.error({
                                                message: 'không load được danh mục của tin tức Sinh viên 5 tốt này'
                                            })
                                        }
                                    } catch (error) {
                                        notification.error({
                    ...
                    {text.status !== STATUS_DELETED &&
                        <Tooltip title="Xóa">
                            <div style={{ color: 'red', cursor: 'pointer' }}
                                onClick={() => {
                                    Modal.confirm({
                                        title: 'Bạn có muốn xóa tin tức Sinh viên 5 tốt?',
    ...
    useEffect(() => {
        if (isErrorNews) {
            notification.error({
                message: 'không tải được danh sách tin tức Sinh viên 5 tốt',
                duration: 1.5
            })
        ...
        if (isErrorCategory) {
            notification.error({
                message: 'không tải được danh sách danh mục tin tức Sinh viên 5 tốt',
                duration: 1.5
            })
    ...
    const onDelete = async (values: News) => {
        ...
        if (response.status === RESPONSE_SUCCESS) {
            ...
            message.success('Xóa tin tức Sinh viên 5 tốt thành công !')
        ...
        form.validateFields()
            .then(async (values) => {
                ...
                if (result.status === RESPONSE_SUCCESS) {
                    message.success("Cập nhật tin tức Sinh viên 5 tốt thành công")
                    ...
                    if (createCategoryRes.status === RESPONSE_SUCCESS) {
                        message.success("Cập nhật danh mục tin tức Sinh viên 5 tốt thành công!")
                    } else {
                        message.error("Cập nhật danh mục tin tức Sinh viên 5 tốt thất bại!")
                    }
                    descRef?.current?.setContent('')
                } else {
                    message.error("Cập nhật tin tức Sinh viên 5 tốt thất bại!")
                    descRef?.current?.setContent('')
                }
                dispatch(fetchNewsList({
                    pageSize,
    ...
    return (
        <>
            <div id="posts">
                <Space>
                    <Button type="primary" onClick={() => {
                        ...
                        setPostDetailUpdate(undefined)
                        form.resetFields()
                    }}>TẠO TIN TỨC VỀ SINH VIÊN 5 TỐT</Button>
                    <div className="posts-actions">
                        <Space>
                            <Input placeholder="Tìm kiếm tin tức Sinh viên 5 tốt" onChange={((e) => onSearch(e.target.value))} style={{ width: 200 }} />
                    ...
                    <div className="list-post">
                        <div style={{ display: 'flex' }}>
                            <h2 className="list-post-title">
                                Danh sách tin tức Sinh viên 5 tốt
                            </h2>
                        </div>
                        <Table
                            bordered
                            ...
                            pagination={{
                                ...
                                total: newsListTotal,
                                showTotal: (total, range) => `${range[0]}-${range[1]} của ${total} tin tức Sinh viên 5 tốt`
                            }}
                        />
                    </div>
                </div>
            ...
            <Modal
                maskClosable={false}
                centered
                title={postDetailUpdate ? 'Sửa tin tức Sinh viên 5 tốt' : 'Tạo tin tức Sinh viên 5 tốt'}
                visible={isModalOpen}
                onOk={onFinish}
                onCancel={() => {
                    setIsModalOpen(false)
                            ...
                            <Col xl={6} md={8} xs={24}>
                                <div className="posts-modal-add-actions">
                                    ...
                                    <div className="posts-modal-add-actions-body">
                                        ...
                                        <div className="posts-modal-add-actions-body-item">
                                            <Form.Item initialValue={postDetailUpdate?.avatar} label={<h3>Ảnh bìa tin tức Sinh viên 5 tốt</h3>} name="avatar" >
                                                <UploadAvatarCrop
