...
import { DOMAIN_ID_ALUMNI, PAGE_SIZE, RESPONSE_FAILED, RESPONSE_SUCCESS, statuses, STATUS_DELETED, STATUS_PRIVATE, STATUS_PUBLIC, STATUS_OPEN, DOMAIN_ID_CDBK, CONTENT_TYPE_NORMAL, CONTENT_TYPE_INTRODUCE } from '@/utils/contrants';
...
function IntroducePageView() {
                                ...
                                if (category !== "0") {
                                    try {
                                        ...
                                        if (categoryOfnews.status === RESPONSE_SUCCESS) {
                                            ...
                                            notification.error({
                                                message: 'không load được danh mục của giới thiệu này'
                                            })
                                        }
                                    } catch (error) {
                                        notification.error({
                    ...
                    {text.status !== STATUS_DELETED &&
                        <Tooltip title="Xóa">
                            <div style={{ color: 'red', cursor: 'pointer' }}
                                onClick={() => {
                                    Modal.confirm({
                                        title: 'Bạn có muốn xóa giới thiệu?',
    ...
    useEffect(() => {
        if (category === "0") { // all
            dispatch(fetchNewsList({
                ...
                status: status,
                contentType: CONTENT_TYPE_INTRODUCE
            }))
        } else {
            // filter
    ...
    useEffect(() => {
        if (isErrorNews) {
            notification.error({
                message: 'không tải được danh sách giới thiệu',
                duration: 1.5
            })
        ...
        if (isErrorCategory) {
            notification.error({
                message: 'không tải được danh sách danh mục giới thiệu',
                duration: 1.5
            })
    ...
    const onDelete = async (values: News) => {
        ...
        if (response.status === RESPONSE_SUCCESS) {
            ...
            message.success('Xóa giới thiệu thành công !')
        ...
        form.validateFields()
            .then(async (values) => {
                ...
                if (result.status === RESPONSE_SUCCESS) {
                    message.success("Cập nhật giới thiệu thành công")
                    ...
                    if (createCategoryRes.status === RESPONSE_SUCCESS) {
                        message.success("Cập nhật danh mục giới thiệu thành công!")
                    } else {
                        message.error("Cập nhật danh mục giới thiệu thất bại!")
                    }
                    descRef?.current?.setContent('')
                } else {
                    message.error("Cập nhật giới thiệu thất bại!")
                    descRef?.current?.setContent('')
                }
                dispatch(fetchNewsList({
                    pageSize,
    ...
    return (
        <>
            <div id="posts">
                <Space>
                    <Button type="primary" onClick={() => {
                        ...
                        setPostDetailUpdate(undefined)
                        form.resetFields()
                    }}>TẠO BÀI GIỚI THIỆU</Button>
                    <div className="posts-actions">
                        <Space>
                            <Input placeholder="Tìm kiếm giới thiệu" onChange={((e) => onSearch(e.target.value))} style={{ width: 200 }} />
                    ...
                    <div className="list-post">
                        ...
                        <Table
                            ...
                            pagination={{
                                ...
                                total: newsListTotal,
                                showTotal: (total, range) => `${range[0]}-${range[1]} của ${total} giới thiệu`
                            }}
                        />
                    </div>
                </div>
            ...
            <Modal
                maskClosable={false}
                centered
                title={postDetailUpdate ? 'Sửa giới thiệu' : 'Tạo giới thiệu'}
                visible={isModalOpen}
                onOk={onFinish}
                onCancel={() => {
                    setIsModalOpen(false)
                            ...
                            <Col xl={6} md={8} xs={24}>
                                <div className="posts-modal-add-actions">
                                    ...
                                    <div className="posts-modal-add-actions-body">
                                        ...
                                        <div className="posts-modal-add-actions-body-item">
                                            <Form.Item initialValue={postDetailUpdate?.avatar} label={<h3>Ảnh bìa giới thiệu</h3>} name="avatar" >
                                                <UploadAvatarCrop
