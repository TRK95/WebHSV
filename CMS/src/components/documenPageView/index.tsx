...
import { apiAddOrRemoveCategory, apiGetcategoryOfNews, apiGetNewsBySlug, apiGetNewsCategory, apiGetNewsCategoryBySlug, apiUpdateNews } from '@/api/newsApi';
...
import { DOMAIN_ID_ALUMNI, PAGE_SIZE, RESPONSE_FAILED, RESPONSE_SUCCESS, statuses, STATUS_DELETED, STATUS_PRIVATE, STATUS_PUBLIC, STATUS_OPEN, DOMAIN_ID_CDBK, CONTENT_TYPE_FILE } from '@/utils/contrants';
...
import UploadFileNews from '../UploadFile';
...
function DocumentPageView() {
    ...
    const [categoryDocument, setCategoryDocument] = useState<string>("0");
    const [dataDocUpload, setDataDocUpload] = useState<string>()
                                ...
                                if (category !== "0") {
                                    try {
                                        ...
                                        if (categoryOfnews.status === RESPONSE_SUCCESS) {
                                            ...
                                            notification.error({
                                                message: 'Không load được danh mục của văn bản này'
                                            })
                                        }
                                    } catch (error) {
                                        notification.error({
    ...
    //     const document = await apiGetNewsCategory({
    //         parentId: -1,
    //         status: 1,
    //         type: 3
    //     })
    //     setCategoryDocument(document.data[0]?.id ?? 0)

    //     dispatch(fetchNewsCategory({
    //         parentId: document.data[0]?.id ?? 0,
    ...
    useEffect(() => {
        if (isErrorNews) {
            notification.error({
                message: 'Không tải được danh sách văn bản',
                duration: 1.5
            })
        ...
        if (isErrorCategory) {
            notification.error({
                message: 'Không tải được danh sách danh mục văn bản',
                duration: 1.5
            })
            ...
            .then(async (values) => {
                ...
                if (result.status === RESPONSE_SUCCESS) {
                    ...
                    if (createCategoryRes.status === RESPONSE_SUCCESS) {
                        message.success("Cập nhật danh mục văn bản thành công!")
                    } else {
                        message.error("Cập nhật danh mục văn bản thất bại!")
                    }
                } else {
                    message.error("Cập nhật văn bản thất bại!")
                }
                dispatch(fetchNewsList({
                    pageSize,
                    offset: (page * pageSize),
    ...
    const changeCategory = async (value) => {
        const childCategoryData = await apiGetNewsCategory({
            parentId: value,
            status: STATUS_PUBLIC
        })
        if (childCategoryData.data) {
            setChildCategoryOptions(childCategoryData.data.map(item => {
                return {
                    // value: item.id || 0,
                    value: item._id || "0",
                    label: item.title
    ...
    return (
        <>
            <div id="posts">
                <Space>
                    ...
                    <div className="posts-actions">
                        <Space>
                            <Input placeholder="Tìm kiếm văn bản" onChange={((e) => onSearch(e.target.value))} style={{ width: 200 }} />
            ...
            <Modal
                maskClosable={false}
                centered
                title={postDetailUpdate ? 'Sửa văn bản' : 'Tạo văn bản'}
                visible={isModalOpen}
                onOk={onFinish}
                onCancel={() => {
                    setIsModalOpen(false)
                    ...
                    >
                        <Row gutter={{ xl: 24, md: 16, xs: 0 }}>
                            <Col xl={18} md={16} xs={24}>
                                ...
                                <Form.Item label={<h3>File văn bản</h3>} name="file" >
                                    <UploadFileNews
                                        defaultUrl={postDetailUpdate?.docUrl}
                                        onChangeUrl={(value) => setDataDocUpload(value)}
                                        width={3}
                                        height={2}
                                        baseFolder='documents/files'
                                    />
                                    {/* <Upload>
                            ...
                            <Col xl={6} md={8} xs={24}>
                                <div className="posts-modal-add-actions">
                                    ...
                                    <div className="posts-modal-add-actions-body">
                                        ...
                                        <div className="posts-modal-add-actions-body-item">
                                            <Form.Item initialValue={postDetailUpdate?.avatar} label={<h3>Ảnh bìa văn bản</h3>} name="avatar" >
                                                <UploadAvatarCrop
                                                    defaultUrl={postDetailUpdate?.avatar}
                                                    onChangeUrl={(value) => setDataupload(value)}
                                                    width={3}
                                                    height={2}
                                                    baseFolder='documents/images'
                                                />
                                            </Form.Item>
                                        </div>
                                    </div>
