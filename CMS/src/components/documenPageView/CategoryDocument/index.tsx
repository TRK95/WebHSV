...
import { DOMAIN_ID_ALUMNI, RESPONSE_FAILED, RESPONSE_SUCCESS, statuses, STATUS_DELETED, STATUS_PRIVATE, STATUS_PUBLIC, TYPE_CLUB, DOMAIN_ID_CDBK } from '@/utils/contrants';
...
function CategoryDocument() {
    ...
    const [categoryDocument, setCategoryDocument] = useState<string>("-1");
    ...
    useEffect(() => {
        if (newsCategoryDetail) {
            // const parentId = Number(newsCategoryDetail.parentId) === categoryDocument ? null : newsCategoryDetail.parentId
            const parentId = newsCategoryDetail.parentId === categoryDocument ? null : newsCategoryDetail.parentId
        ...
        form.validateFields()
            .then(async (values: NewsCategoryModel) => {
                ...
                const bodyReq = {
                    ...
                    slug: values?.slug ?? '',
                    parentId: values?.parentId ?? categoryDocument,
                ...
                if (newsCategoryDetail) {
                    if (categoryDocument) {
                        ...
                        message.error("Tạo danh mục văn bản thất bại!")
                    }
                } else {
                    if (categoryDocument) {
                        ...
                        if (createRes.status === RESPONSE_SUCCESS) {
                            ...
                            message.success("Tạo danh mục văn bản thành công!")
                    ...
                    {text.status !== STATUS_DELETED &&
                        <Tooltip title="Xóa">
                            <div style={{ color: 'red', cursor: 'pointer' }}
                                onClick={() => {
                                    Modal.confirm({
                                        title: 'Bạn có muốn xóa danh mục văn bản?',
    ...
    return (
        <>
            ...
            <Modal
                maskClosable={false}
                centered
                title={!!newsCategoryDetail ? 'Sửa Danh mục văn bản' : 'Tạo Danh mục văn bản'}
                getContainer={false}
                visible={isModalOpen}
                onOk={onFinish}
                onCancel={() => {
