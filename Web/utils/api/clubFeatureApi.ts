...
export const apiGetClubFeatureChildByClubSlug = async (reqQuery: { slug: string | undefined, status: number }): Promise<{ data: any | [], total: number, status: number }> => {
    const url = 'clubFeatureChild/getClubFeatureChildByClubSlug';
    const res = await POST_API({ url, reqQuery });
    if (res.status !== 200) return {
        data: [],
        total: 0,
